...
extern "C" 
int Compare_Result(size_t optionCount,
                  float *h_Call_v, float *h_Put_v, float *h_Call_s, float *h_Put_s){
        double deltaCall = 0, deltaPut = 0, sumCall = 0, sumPut = 0;
        for(unsigned int i = 0; i < optionCount; i++)
        {
            sumCall = fabs(h_Call_s[i]);
            sumPut  = fabs(h_Put_s[i]);
            deltaCall = fabs(h_Call_v[i] - h_Call_s[i]) / sumCall;
            deltaPut  = fabs(h_Put_v[i]  - h_Put_s[i] ) / sumPut;
            if(deltaCall > 1.0e-6){
                fprintf(stdout, "++++ index : %u, h_Call_v[i] : %f, h_Call_s[i] : %f\n", i, h_Call_v[i] , h_Call_s[i]);
                return -1;
            }
            if(deltaPut > 1.0e-6){
                fprintf(stdout, "++++ index : %u, h_Put_v[i] : %f, h_Put_s[i] : %f\n", i, h_Put_v[i] , h_Put_s[i]);
                return -1;
            }
        }
        return 0;
